...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (init.method && init.method !== 'GET') {
    ...
    console.log('CSRF Token:', csrfToken ? 'Found' : 'NOT FOUND', '| Cookie:', document.cookie.substring(0, 100))
    if (csrfToken) {
  ...
  const skipRetry = path === '/auth/login/' || path === '/auth/signup/' || path === '/auth/token/refresh/'
  ...
  if (!res.ok) {
    ...
    throw Object.assign(new Error((data as any).detail || (data as any).message || res.statusText), {
      status: res.status,
      data,
    })
  }
