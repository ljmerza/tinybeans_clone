...
def redacted_value(key: str, value: Any) -> Any:
    if isinstance(value, MutableMapping):
        return {
            nested_key: redacted_value(f"{key}.{nested_key}", nested_val) for nested_key, nested_val in value.items()
        }
...
class LoggingContextFilter(std_logging.Filter):
    ...
    def filter(self, record: std_logging.LogRecord) -> bool:
        ...
        record.extra_data = (
            _scrub_payload(context.get("extra", {})) if isinstance(context.get("extra"), Mapping) else None
        )
        return True
