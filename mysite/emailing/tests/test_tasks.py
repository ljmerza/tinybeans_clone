...
from emailing.tasks import register_email_template, send_email_task, _TEMPLATE_REGISTRY
from emailing.mailers import MailerConfigurationError, MailerSendError, send_via_mailjet
...
class EmailTaskTests(TestCase):
    ...
    @patch('emailing.tasks.send_mail')
    @patch('emailing.tasks.logger')
...
class MailjetMailerErrorTests(TestCase):
    ...
    @patch('emailing.mailers.requests.post')
