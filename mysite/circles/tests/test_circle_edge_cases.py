...
class CircleViewEdgeCaseTests(TestCase):
    ...
    def test_circle_creation_with_extremely_long_name(self):
        ...
        response = self.client.post(reverse("user-circle-list"), {"name": long_name}, format="json")

        # Expect either 405 (method not allowed) or 400 (validation error) or 404 (not found)
        self.assertIn(
            response.status_code,
            [status.HTTP_405_METHOD_NOT_ALLOWED, status.HTTP_400_BAD_REQUEST, status.HTTP_404_NOT_FOUND],
        )
    ...
    def test_remove_last_admin(self):
        ...
        self.assertIn(
            response.status_code, [status.HTTP_204_NO_CONTENT, status.HTTP_400_BAD_REQUEST, status.HTTP_403_FORBIDDEN]
        )
    ...
    def test_accept_expired_invitation(self):
        ...
        response = self.client.post(
            reverse("circle-invitation-accept"), {"invitation_id": str(invitation.id)}, format="json"
        )
    ...
    def test_multiple_pending_invitations_same_email(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "invitee@example.com"}, format="json"
        )

        # Should either succeed or fail gracefully
        self.assertIn(
            response.status_code, [status.HTTP_202_ACCEPTED, status.HTTP_400_BAD_REQUEST, status.HTTP_409_CONFLICT]
        )
