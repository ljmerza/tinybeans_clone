...
class InvitationSerializerTests(TestCase):
    ...
    def test_role_validation(self):
        ...
        serializer = CircleInvitationCreateSerializer(
            data={"email": "test@example.com", "role": "admin"}, context={"circle": self.circle}
        )
        ...
        # Valid member role
        serializer = CircleInvitationCreateSerializer(
            data={"email": "test2@example.com", "role": "member"}, context={"circle": self.circle}
        )
        self.assertTrue(serializer.is_valid())
        ...
        serializer = CircleInvitationCreateSerializer(
            data={"email": "test3@example.com"}, context={"circle": self.circle}
        )
        self.assertTrue(serializer.is_valid())
        self.assertEqual(serializer.validated_data["role"], UserRole.CIRCLE_MEMBER)
        ...
        # Invalid role
        serializer = CircleInvitationCreateSerializer(
            data={"email": "test4@example.com", "role": "invalid"}, context={"circle": self.circle}
        )
    ...
    def test_existing_user_lookup(self):
        ...
        existing = User.objects.create_user(email="lookup@example.com", password="password123")
        serializer = CircleInvitationCreateSerializer(
            data={"email": "lookup@example.com"}, context={"circle": self.circle}
        )
        self.assertTrue(serializer.is_valid())
    ...
    def test_requires_email(self):
        ...
        serializer = CircleInvitationCreateSerializer(data={}, context={"circle": self.circle})
        self.assertFalse(serializer.is_valid())
