...
from mysite.circles.models import Circle, CircleInvitation, CircleInvitationStatus, CircleMembership
...
class InvitationAdminPermissionTests(TestCase):
    ...
    def test_admin_can_create_invitations_with_roles(self, mock_store_token, mock_delay):
        ...
        # Test default role
        response = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "default@example.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED, response.json())
        payload = response.json()["data"]["invitation"]
        self.assertFalse(payload["existing_user"])
    ...
    def test_admin_can_invite_existing_users(self, mock_store_token, mock_delay):
        ...
        existing_user = User.objects.create_user(email="target@example.com", password="password123")

        response = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "target@example.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED, response.json())
        payload = response.json()["data"]["invitation"]
    ...
    def test_create_requires_email(self, mock_store_token, mock_delay):
        ...
        response = self.client.post(reverse("circle-invitation-create", args=[self.circle.id]), {}, format="json")
    ...
    @override_settings(
        RATELIMIT_ENABLE=True, CIRCLE_INVITE_CIRCLE_LIMIT=1, CIRCLE_INVITE_CIRCLE_LIMIT_WINDOW_MINUTES=60
    )
    @patch("mysite.circles.services.invitation_service.send_email_task.delay")
    @patch("mysite.circles.services.invitation_service.store_token")
    def test_circle_rate_limit(self, mock_store_token, mock_delay):
        ...
        first = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "first@example.com"}, format="json"
        )
        self.assertEqual(first.status_code, status.HTTP_202_ACCEPTED, first.json())

        second = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "second@example.com"}, format="json"
        )
    ...
    def test_member_cannot_create_invitations(self):
        ...
        self.client.force_authenticate(user=self.member)
        response = self.client.post(
            reverse("circle-invitation-create", args=[self.circle.id]), {"email": "test@example.com"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_invalid_role_rejected(self):
        ...
        payload = response.json()
        fields = [
            message.get("context", {}).get("field") for message in payload.get("messages", []) if message.get("context")
        ]
