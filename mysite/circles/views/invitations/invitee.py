    ...
    def post(self, request, invitation_id):
        ...
        if invitation.status != CircleInvitationStatus.PENDING:
            return error_response(
                "invitation_not_pending", [create_message("errors.invitation_not_pending")], status.HTTP_400_BAD_REQUEST
            )
...
class CircleInvitationAcceptView(APIView):
    ...
    @extend_schema(
        description=(
            "Begin onboarding for a circle invitation using the one-time link token. Returns metadata and "
            "an onboarding token for account creation."
        ),
    ...
    def post(self, request):
        ...
        if not payload:
            return error_response(
                "token_invalid_expired", [create_message("errors.token_invalid_expired")], status.HTTP_400_BAD_REQUEST
            )
        ...
        if not invitation:
            return error_response(
                "invitation_not_found", [create_message("errors.invitation_not_found")], status.HTTP_404_NOT_FOUND
            )

        # Verify email matches
        if invitation.email.lower() != payload.get("email", "").lower():
            return error_response(
                "invitation_mismatch", [create_message("errors.invitation_mismatch")], status.HTTP_400_BAD_REQUEST
            )
...
class CircleInvitationFinalizeView(APIView):
    ...
    @extend_schema(
        description=(
            "Finalize circle invitation onboarding using an onboarding token once the invitee is authenticated."
        ),
    ...
    def post(self, request):
        ...
        if not email_match and not invited_match:
            return error_response(
                "invitation_mismatch", [create_message("errors.invitation_mismatch")], status.HTTP_403_FORBIDDEN
            )
