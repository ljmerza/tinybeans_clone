...
class User(AbstractUser):
    ...
    google_id = models.CharField(
        max_length=100, unique=True, null=True, blank=True, db_index=True, help_text="Google user ID from OAuth"
    )
    google_email = models.EmailField(null=True, blank=True, help_text="Email from Google (for debugging/tracking)")
    password_login_enabled = models.BooleanField(
        default=True, help_text="Whether the user can authenticate via password"
    )
    ...
    google_linked_at = models.DateTimeField(null=True, blank=True, help_text="When Google account was linked")
    ...
    circle_onboarding_updated_at = models.DateTimeField(
        null=True, blank=True, help_text="Last time the circle onboarding status changed"
    )
