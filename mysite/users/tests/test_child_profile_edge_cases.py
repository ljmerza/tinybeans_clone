...
class ChildProfileViewEdgeCaseTests(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_user(
            email="parent@example.com", password="password123", role=UserRole.CIRCLE_ADMIN
        )
        ...
        self.child = ChildProfile.objects.create(circle=self.circle, display_name="Test Child")
    ...
    def test_upgrade_request_with_invalid_email(self):
        ...
        data = response.json()
        fields = [
            message.get("context", {}).get("field") for message in data.get("messages", []) if message.get("context")
        ]
