...
class UserModelTests(TestCase):
    ...
    def test_superuser_validation_errors(self):
        ...
        with self.assertRaises(ValueError) as cm:
            User.objects.create_superuser(email="admin2@example.com", password="adminpass", is_superuser=False)
    ...
    def test_needs_circle_onboarding_completed_after_membership(self):
        ...
        Circle.objects.create(name="Family", created_by=user)
...
class CircleMembershipModelTests(TestCase):
    ...
    def test_create_membership(self):
        ...
        membership = CircleMembership.objects.create(
            user=self.member, circle=self.circle, role=UserRole.CIRCLE_MEMBER, invited_by=self.admin
        )
    ...
    def test_membership_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            CircleMembership.objects.create(user=self.member, circle=self.circle, role=UserRole.CIRCLE_ADMIN)

    def test_membership_str_representation(self):
        """Test membership string representation."""
        membership = CircleMembership.objects.create(user=self.member, circle=self.circle, role=UserRole.CIRCLE_MEMBER)
...
class ChildProfileModelTests(TestCase):
    ...
    def test_create_child_profile(self):
        ...
        child = ChildProfile.objects.create(
            circle=self.circle, display_name="Little One", birthdate=date(2015, 5, 10), pronouns="they/them"
        )
    ...
    def test_child_profile_str_representation(self):
        ...
        child = ChildProfile.objects.create(circle=self.circle, display_name="Little One")
    ...
    def test_log_upgrade_event(self):
        ...
        child.log_upgrade_event(
            ChildUpgradeEventType.REQUEST_INITIATED, performed_by=self.admin, metadata={"email": "parent@example.com"}
        )
...
class UserNotificationPreferencesModelTests(TestCase):
    ...
    def test_unique_constraint(self):
        ...
        UserNotificationPreferences.objects.create(user=self.user, circle=self.circle)
    ...
    def test_str_representation(self):
        ...
        circle_prefs = UserNotificationPreferences.objects.create(user=self.user, circle=self.circle)
...
class CircleInvitationModelTests(TestCase):
    ...
    def test_create_invitation(self):
        ...
        invitation = CircleInvitation.objects.create(
            circle=self.circle, email="invitee@example.com", invited_by=self.admin, role=UserRole.CIRCLE_MEMBER
        )
    ...
    def test_invitation_str_representation(self):
        ...
        invitation = CircleInvitation.objects.create(
            circle=self.circle, email="invitee@example.com", invited_by=self.admin
        )
...
class ChildGuardianConsentModelTests(TestCase):
    def setUp(self):
        self.admin = User.objects.create_user(email="admin@example.com", password="password123")
        self.circle = Circle.objects.create(name="Family", created_by=self.admin)
        self.child = ChildProfile.objects.create(circle=self.circle, display_name="Little One")
