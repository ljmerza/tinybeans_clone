...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--email",
            default=os.environ.get("TINYBEANS_EMAIL"),
            help="Tinybeans account email (or TINYBEANS_EMAIL env var)",
        )
        parser.add_argument(
            "--password",
            default=os.environ.get("TINYBEANS_PASSWORD"),
            help="Tinybeans account password (or TINYBEANS_PASSWORD env var; prompted if omitted)",
        )
        parser.add_argument(
            "--token",
            default=os.environ.get("TINYBEANS_ACCESS_TOKEN"),
            help="Existing Tinybeans access token instead of email/password (requires --owner)",
        )
        ...
        parser.add_argument(
            "--owner", help="Email of the local user who owns imported circles (default: the Tinybeans account email)"
        )
        parser.add_argument(
            "--limit", type=int, default=None, help="Stop after importing this many new entries (for testing)"
        )
        parser.add_argument(
            "--dry-run", action="store_true", help="Report what would be imported without writing anything"
        )
        parser.add_argument(
            "--sync-thumbnails",
            action="store_true",
    ...
    def _execute(self, options):
        ...
        if c["errors"]:
            self.stdout.write(
                self.style.WARNING("Some entries failed (see warnings above); re-run the command to retry them.")
            )
    ...
    def _resolve_owner(self, options, login_user):
        ...
        self.stdout.write(
            self.style.WARNING(
                f"Created local owner account {user.email} with no password (use password reset to enable login)."
            )
        )
        return user
    ...
    def _sync_journal(self, journal, start_ms, end_ms):
        ...
        for entry in self.client.iter_entries(
            ...
            start_ms,
            end_ms,
            updated_since_ms=self.updated_since_ms,
        ):
            ...
            try:
                ...
                self.stderr.write(self.style.WARNING(f"Failed to import entry {entry.get('id')}: {exc}"))
            processed += 1
            if processed % 100 == 0:
    ...
    def _ensure_user(self, remote_user, circle):
        ...
        if record:
            ...
            ).strip().lower() or f"tinybeans-{remote_id}@{PLACEHOLDER_EMAIL_DOMAIN}"
            user = User.objects.filter(email__iexact=email).first()
            if not user:
                self.counts["users"] += 1
                if self.dry:
    ...
    def _generate_thumbnails(self, media_id, source_key=None):
        try:
            ...
            self.stderr.write(
                self.style.WARNING(f"Thumbnail generation for media {media_id} could not be started: {exc}")
            )
        ...
        for comment in items or []:
            ...
            if (comment.get("repliesCount") or 0) > 0 and depth < MAX_REPLY_DEPTH:
                try:
                    ...
                    self.counts["errors"] += 1
                    self.stderr.write(self.style.WARNING(f"Could not fetch replies for comment {comment_id}: {exc}"))
                    continue
