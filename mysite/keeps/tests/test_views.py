...
from mysite.keeps.models import Keep, KeepComment, KeepReaction, KeepType
...
def circle(user):
    ...
    circle = Circle.objects.create(name="Test Family", created_by=user)
...
def other_circle(other_user):
    ...
    circle = Circle.objects.create(name="Other Family", created_by=other_user)
...
def keep(user, circle):
    ...
    return Keep.objects.create(
        circle=circle, created_by=user, keep_type=KeepType.NOTE, title="Test Memory", description="A wonderful memory"
    )
...
class TestKeepListCreateView:
    ...
    def test_list_keeps_excludes_other_circles(self, api_client, user, circle, keep, other_user, other_circle):
        ...
        Keep.objects.create(circle=other_circle, created_by=other_user, keep_type=KeepType.NOTE, title="Private Memory")
    ...
    def test_create_keep_requires_circle_membership(self, api_client, user, other_circle):
        ...
        assert any(
            m.get("context", {}).get("field") == "circle" and m["i18n_key"] == "errors.circle_membership_required"
    ...
    def test_filter_by_keep_type(self, api_client, user, circle):
        ...
        Keep.objects.create(circle=circle, created_by=user, keep_type=KeepType.NOTE, title="Note")
        Keep.objects.create(circle=circle, created_by=user, keep_type=KeepType.MEDIA, title="Photo")
    ...
    def test_filter_by_tag(self, api_client, user, circle):
        ...
        Keep.objects.create(
            circle=circle, created_by=user, keep_type=KeepType.NOTE, title="Summer", tags="summer,vacation"
        )
        Keep.objects.create(circle=circle, created_by=user, keep_type=KeepType.NOTE, title="Winter", tags="winter,snow")
