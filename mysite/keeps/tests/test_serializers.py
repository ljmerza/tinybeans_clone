...
class TestKeepSerializer:
    ...
    def test_tag_list_with_no_tags(self, user, circle):
        ...
        keep = Keep.objects.create(circle=circle, created_by=user, keep_type=KeepType.NOTE, title="No Tags", tags="")
        serializer = KeepSerializer(instance=keep)
        data = serializer.data
    ...
    def test_reaction_count(self, keep):
        ...
        # Add reaction
        KeepReaction.objects.create(keep=keep, user=keep.created_by, reaction_type="like")
    ...
    def test_comment_count(self, keep):
        ...
        # Add comment
        KeepComment.objects.create(keep=keep, user=keep.created_by, comment="Great memory!")
...
class TestKeepCreateSerializer:
    ...
    def test_validation_requires_circle(self, user):
        ...
        data = {"keep_type": KeepType.NOTE, "title": "No Circle"}
    ...
    def test_validation_requires_valid_keep_type(self, user, circle):
        ...
        data = {"circle": str(circle.id), "keep_type": "invalid_type", "title": "Bad Type"}
...
class TestKeepDetailSerializer:
    ...
    def test_includes_milestone_data(self, user, circle):
        ...
        keep = Keep.objects.create(circle=circle, created_by=user, keep_type=KeepType.MILESTONE, title="First Steps")

        Milestone.objects.create(
            keep=keep, milestone_type=MilestoneType.FIRST_STEPS, age_at_milestone="12 months", is_first_time=True
        )
