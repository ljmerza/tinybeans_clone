...
from mysite.keeps.models import Keep, KeepComment, KeepMedia, KeepReaction, KeepType, Milestone, MilestoneType
...
class KeepModelTest(TestCase):
    ...
    def test_keep_str_representation(self):
        ...
        keep = Keep.objects.create(
            circle=self.circle, created_by=self.user, keep_type=KeepType.MEDIA, title="Beach Day"
        )
    ...
    def test_keep_without_title_str_representation(self):
        ...
        keep = Keep.objects.create(circle=self.circle, created_by=self.user, keep_type=KeepType.MILESTONE)
    ...
    def test_tag_list_property(self):
        ...
        keep = Keep.objects.create(
            circle=self.circle, created_by=self.user, keep_type=KeepType.MEDIA, tags="beach,summer,vacation,family"
        )
    ...
    def test_milestone_creation(self):
        ...
        keep = Keep.objects.create(
            circle=self.circle, created_by=self.user, keep_type=KeepType.MILESTONE, title="First Steps"
        )
    ...
    def test_keep_media_creation(self):
        ...
        keep = Keep.objects.create(
            circle=self.circle, created_by=self.user, keep_type=KeepType.MEDIA, title="Family Photo"
        )
    ...
    def test_keep_media_properties(self):
        ...
        keep = Keep.objects.create(
            circle=self.circle, created_by=self.user, keep_type=KeepType.MEDIA, title="Mixed Media Keep"
        )
...
class KeepAdminPermissionsTest(TestCase):
    ...
    def setUp(self):
        ...
        # Create users
        self.admin_user = User.objects.create_user(email="admin@example.com", password="adminpass123")
        self.regular_user = User.objects.create_user(email="regular@example.com", password="regularpass123")

        # Create circle
        self.circle = Circle.objects.create(name="Test Family", created_by=self.admin_user)
        # Membership for admin_user is auto-created by the post_save signal on Circle

        # Create membership for regular user
        CircleMembership.objects.create(user=self.regular_user, circle=self.circle, role=UserRole.CIRCLE_MEMBER)
        ...
        self.reaction = KeepReaction.objects.create(keep=self.keep, user=self.regular_user, reaction_type="like")

        # Create a comment by regular user
        self.comment = KeepComment.objects.create(keep=self.keep, user=self.regular_user, comment="Great memory!")
    ...
    def test_regular_user_cannot_modify_others_content(self):
        ...
        CircleMembership.objects.create(user=other_user, circle=self.circle, role=UserRole.CIRCLE_MEMBER)
