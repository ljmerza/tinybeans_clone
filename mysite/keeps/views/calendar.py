...
class KeepCalendarView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(description="Calendar entries for the requested month"),
            400: OpenApiResponse(description="Missing or invalid month parameter"),
    ...
    def get(self, request):
        ...
        if not match or not 1 <= int(match.group(2)) <= 12:
            return error_response("invalid_month", messages=[create_message("errors.invalid_month")], status_code=400)
        ...
        if circle_slug:
            try:
                ...
                return error_response(
                    "circle_not_found", messages=[create_message("errors.circle_not_found")], status_code=404
                )
        ...
        for keep in keeps:
            photos = [
                ...
                if media.media_type == "photo" or (media.media_type == "video" and media.thumbnails_generated)
            ]
            if not photos:
                continue
            entries.append(
