...
class KeepMediaListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return KeepMedia.objects.filter(keep__circle__in=user_circles).select_related("keep")

    @extend_schema(
        summary="List keep media files",
        description="List all media files (photos and videos) attached to keeps in user's circles.",
        responses={200: OpenApiResponse(response=KeepMediaSerializer(many=True), description="List of media files")},
    )
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=KeepMediaSerializer, description="Media file uploaded successfully"),
            400: OpenApiResponse(description="Validation error or unsupported file type"),
        },
    )
...
class KeepMediaDetailView(generics.RetrieveDestroyAPIView):
    ...
    def perform_destroy(self, instance):
        ...
        if instance.keep.created_by != user and not is_circle_admin(user, instance.keep.circle):
            raise permissions.PermissionDenied("You can only delete media from keeps you created or as a circle admin.")
        instance.delete()
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=KeepMediaSerializer, description="Media file details"),
            404: OpenApiResponse(description="Media file not found or not accessible"),
        },
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
    ...
    @extend_schema(
        ...
        responses={
            ...
            403: OpenApiResponse(description="Permission denied - only keep creators or circle admins can delete"),
            404: OpenApiResponse(description="Media file not found or not accessible"),
        },
    )
