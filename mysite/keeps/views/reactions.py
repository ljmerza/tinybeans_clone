...
class KeepReactionListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return KeepReaction.objects.filter(keep__circle__in=user_circles).select_related("user", "keep")
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=KeepReactionSerializer(many=True), description="List of reactions to keeps")
        },
    )
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=KeepReactionSerializer, description="Reaction created successfully"),
            400: OpenApiResponse(description="Validation error or duplicate reaction"),
        },
    )
...
class KeepReactionDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if reaction.user != user and not is_circle_admin(user, reaction.keep.circle):
            raise permissions.PermissionDenied("You can only update your own reactions or as a circle admin.")
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if instance.user != user and not is_circle_admin(user, instance.keep.circle):
            raise permissions.PermissionDenied("You can only delete your own reactions or as a circle admin.")
        instance.delete()
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=KeepReactionSerializer, description="Reaction details"),
            404: OpenApiResponse(description="Reaction not found or not accessible"),
        },
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
    ...
    @extend_schema(
        ...
        request=KeepReactionSerializer,
        responses={
            200: OpenApiResponse(response=KeepReactionSerializer, description="Reaction updated successfully"),
    ...
    @extend_schema(
        ...
        description=(
            "Partially update a reaction to a keep. Only the reaction creator or circle admins can update reactions."
        ),
        request=KeepReactionSerializer,
        responses={
            200: OpenApiResponse(response=KeepReactionSerializer, description="Reaction updated successfully"),
    ...
    @extend_schema(
        ...
        responses={
            204: OpenApiResponse(description="Reaction deleted successfully"),
