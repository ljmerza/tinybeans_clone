...
class KeepListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Keep.objects.filter(circle__in=user_circles)
            .select_related("circle", "created_by", "milestone", "milestone__child_profile")
            .prefetch_related("media_files", "reactions__user", "comments__user")
        )
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=KeepSerializer(many=True), description="Paginated list of keeps from user's circles"
            )
        },
    )
    ...
    @extend_schema(
        ...
        responses={
            201: OpenApiResponse(response=KeepSerializer, description="Keep created successfully"),
            400: OpenApiResponse(description="Validation error or circle membership required"),
            403: OpenApiResponse(description="Not a member of the specified circle"),
        },
    )
...
class KeepDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        if keep.created_by != user and not is_circle_admin(user, keep.circle):
            raise permissions.PermissionDenied("You can only update keeps you created or as a circle admin.")
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if instance.created_by != user and not is_circle_admin(user, instance.circle):
            raise permissions.PermissionDenied("You can only delete keeps you created or as a circle admin.")
        instance.delete()
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=KeepDetailSerializer, description="Detailed keep information with all related data"
            ),
            404: OpenApiResponse(description="Keep not found or not accessible"),
        },
    )
    def get(self, request, *args, **kwargs):
        return super().get(request, *args, **kwargs)
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=KeepSerializer, description="Keep updated successfully"),
    ...
    @extend_schema(
        ...
        responses={
            204: OpenApiResponse(description="Keep deleted successfully"),
...
class KeepByCircleView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=KeepSerializer(many=True), description="List of keeps for the specified circle"
            ),
            404: OpenApiResponse(description="Circle not found or user is not a member"),
        },
    )
    def get(self, request, circle_slug):
        """Get keeps for a specific circle."""
        try:
            circle = Circle.objects.get(slug=circle_slug, memberships__user=request.user)
        ...
        keeps = (
            Keep.objects.filter(circle=circle)
            .select_related("circle", "created_by")
            .prefetch_related("media_files", "reactions", "comments")
...
class KeepByTypeView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(response=KeepSerializer(many=True), description="List of keeps of the specified type"),
            400: OpenApiResponse(description="Invalid or missing keep type parameter"),
        },
    )
    def get(self, request):
        ...
        user_circles = Circle.objects.filter(memberships__user=request.user)

        keeps = (
            Keep.objects.filter(circle__in=user_circles, keep_type=keep_type)
            .select_related("circle", "created_by")
            .prefetch_related("media_files", "reactions", "comments")
            .order_by("-date_of_memory", "-created_at")
        )
