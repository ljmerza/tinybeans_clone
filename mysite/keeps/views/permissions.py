...
class IsCircleMember(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return CircleMembership.objects.filter(user=request.user, circle=circle).exists()
...
class IsCircleAdminOrOwner(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        try:
            membership = CircleMembership.objects.get(user=request.user, circle=circle)
...
def can_user_post_in_circle(user, circle):
    ...
    return CircleMembership.objects.filter(user=user, circle=circle).exists()
