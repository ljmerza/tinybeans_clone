...
from ..models import RecoveryCode, TwoFactorCode
...
class TwoFactorService:
    ...
    def _hash_otp(code: str) -> str:
        ...
        return hmac.new(key.encode("utf-8"), code.encode("utf-8"), hashlib.sha256).hexdigest()
    ...
    def generate_totp_qr_code(user, secret: str) -> dict:
        ...
        return {"uri": uri, "qr_code_image": f"data:image/png;base64,{img_str}", "secret": secret}
    ...
    def verify_otp(user, code, purpose="login") -> bool:
        ...
        try:
            ...
            code_obj = TwoFactorCode.objects.filter(
                user=user, code_hash=code_hash, purpose=purpose, is_used=False
            ).first()
    ...
    def is_rate_limited(user) -> bool:
        ...
        recent_codes = TwoFactorCode.objects.filter(user=user, created_at__gte=since).count()
