...
class AccountLinkingService:
    ...
    def get_or_create_user(self, google_user_info: Dict[str, any]) -> Tuple[User, str]:
        ...
        try:
            ...
            logger.info("OAuth account linked", extra={"user_id": existing_user.id, "google_id": mask_id(google_id)})
    ...
    def link_google_account(self, user: User, google_user_info: Dict[str, any]) -> User:
        ...
        if user.email != google_email:
            raise OAuthError(f"Google email ({google_email}) doesn't match user email ({user.email})")

        # Check if Google ID already linked to different user
        if User.objects.filter(google_id=google_id).exclude(id=user.id).exists():
            raise GoogleAccountAlreadyLinkedError("This Google account is already linked to another user")
        ...
        logger.info("Google account linked to user", extra={"user_id": user.id, "google_id": mask_id(google_id)})
    ...
    def unlink_google_account(self, user: User) -> User:
        ...
        if not user.password_login_enabled:
            raise OAuthError("Cannot unlink Google account without setting a password first")
        ...
        logger.info("Google account unlinked", extra={"user_id": user.id})
