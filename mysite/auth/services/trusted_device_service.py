...
class TrustedDeviceService:
    ...
    def _sign_device_id(device_id: str) -> str:
        ...
        signer = signing.TimestampSigner(key=getattr(settings, "TWOFA_TRUSTED_DEVICE_SIGNING_KEY", settings.SECRET_KEY))
    ...
    def add_trusted_device(user, request, days=None):
        ...
        existing_count = TrustedDevice.objects.filter(user=user, expires_at__gt=timezone.now()).count()

        if existing_count >= max_devices:
            # Remove oldest device
            oldest = (
                TrustedDevice.objects.filter(user=user, expires_at__gt=timezone.now()).order_by("created_at").first()
            )
            if oldest:
                oldest.delete()
    ...
    def _rotate_device(device: TrustedDevice, request) -> TrustedDevice:
        ...
        device.save(
            update_fields=["device_id", "ua_hash", "ip_hash", "expires_at", "user_agent", "ip_address", "last_used_at"]
        )
    ...
    def is_trusted_device(user, token: TrustedDeviceToken, request) -> Tuple[bool, Optional[TrustedDeviceToken]]:
        ...
        now = timezone.now()
        try:
            device = TrustedDevice.objects.get(user=user, device_id=token.device_id, expires_at__gt=now)
    ...
    def remove_trusted_device(user, device_id) -> bool:
        ...
        try:
            ...
            # Log the removal
            TwoFactorAuditLog.objects.create(
                user=user, action="trusted_device_removed", method="device_trust", device_id=device_id, success=True
            )
    ...
    def cleanup_expired_devices():
        ...
        expired_count = TrustedDevice.objects.filter(expires_at__lte=timezone.now()).delete()[0]
