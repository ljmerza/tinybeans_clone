...
class TrustedDevicesListView(APIView):
    ...
    @extend_schema(request={"device_id": "string"}, responses={200: dict})
    def delete(self, request):
        ...
        if not device_id:
            return error_response(
                "device_id_required", [create_message("errors.twofa.device_id_required")], status.HTTP_400_BAD_REQUEST
            )
        ...
        if removed:
            ...
            return error_response(
                "device_not_found", [create_message("errors.twofa.device_not_found")], status.HTTP_404_NOT_FOUND
            )

    @extend_schema(responses={201: TrustedDeviceSerializer})
    def post(self, request):
        ...
        message_key = "notifications.twofa.device_added" if created else "notifications.twofa.device_already_trusted"
        response = success_response(
            {"device": serializer.data, "created": created},
            messages=[create_message(message_key, {"device_name": trusted_device.device_name})],
