...
class TwoFactorSetupView(APIView):
    ...
    @extend_schema(request=TwoFactorSetupSerializer, responses={200: dict})
    def post(self, request):
        ...
        settings_obj, created = TwoFactorSettings.objects.get_or_create(
            user=user, defaults={"preferred_method": method}
        )
        ...
        if method == "totp":
            ...
            # Send OTP
            TwoFactorService.send_otp(user, method=method, purpose="setup")
        ...
        return error_response(
            "invalid_method", [create_message("errors.twofa.invalid_method")], status.HTTP_400_BAD_REQUEST
        )
...
class TwoFactorVerifySetupView(APIView):
    ...
    @extend_schema(request=TwoFactorVerifySetupSerializer, responses={200: dict})
    def post(self, request):
        ...
        try:
            ...
            return error_response(
                "not_initialized", [create_message("errors.twofa.not_initialized")], status.HTTP_400_BAD_REQUEST
            )
