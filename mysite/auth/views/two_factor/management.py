...
class TwoFactorMethodRemoveView(APIView):
    ...
    def delete(self, request, method: str):
        ...
        if became_disabled:
            ...
            messages.append(
                create_message(
                    "notifications.twofa.method_removed_preferred_changed",
                    {"preferred_method": settings_obj.preferred_method.upper()},
                )
            )
...
class TwoFactorDisableRequestView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return error_response(
                "not_enabled", [create_message("errors.twofa.not_enabled")], status.HTTP_400_BAD_REQUEST
            )

        if not settings_obj.is_enabled:
            return error_response(
                "already_disabled", [create_message("errors.twofa.already_disabled")], status.HTTP_400_BAD_REQUEST
            )
        ...
        TwoFactorService.send_otp(user, method=settings_obj.preferred_method, purpose="disable")
        ...
        return success_response(
            ...
            messages=[
                create_message("notifications.twofa.verification_code_sent", {"method": settings_obj.preferred_method})
            ],
        )
...
class TwoFactorDisableView(APIView):
    ...
    @extend_schema(request=TwoFactorDisableSerializer, responses={200: dict})
    def post(self, request):
