...
class RecoveryCodeGenerateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return error_response(
                "not_configured", [create_message("errors.twofa.not_configured")], status.HTTP_400_BAD_REQUEST
            )
...
class RecoveryCodeDownloadView(APIView):
    ...
    def post(self, request):
        ...
        if len(recovery_codes) == 0:
            return error_response(
                "no_recovery_codes", [create_message("errors.twofa.no_recovery_codes")], status.HTTP_400_BAD_REQUEST
            )
