...
class SignupView(APIView):
    ...
    @extend_schema(
        description=(
            "Register a new account and receive an access token (refresh token stored as an HTTP-only "
            "cookie). Email verification is required before creating circles."
        ),
...
class LoginView(APIView):
    ...
    @extend_schema(
        description=(
            "Authenticate with email/password and receive an access token (refresh token stored in "
            "HTTP-only cookie). If 2FA is enabled, returns requires_2fa flag and partial token."
        ),
        request=LoginSerializer,
        responses={
            200: OpenApiResponse(
                response=OpenApiTypes.OBJECT, description="JWT tokens and user payload or 2FA required response"
            )
        },
    )
    ...
    def post(self, request):
                ...
                if twofa_settings.is_enabled:
                    ...
                    if twofa_settings.is_locked():
                        ...
                        return error_response(
                            "account_locked",
                            [create_message("errors.account_locked_2fa", {"retry_after": "later"})],
                    ...
                    if is_trusted:
                        ...
                        response_data = success_response(
                            user_data, messages=[create_message("notifications.auth.login_success")]
                        )
                        set_refresh_cookie(response_data, tokens["refresh"])
                        if rotated_token:
        ...
        response_data = success_response(user_data, messages=[create_message("notifications.auth.login_success")])
        set_refresh_cookie(response_data, tokens["refresh"])
        return response_data
