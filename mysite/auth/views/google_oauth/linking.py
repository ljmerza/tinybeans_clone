...
class GoogleOAuthLinkView(APIView):
    ...
    @method_decorator(
        ratelimit(
            key="user", rate=f"{settings.OAUTH_RATE_LIMIT_MAX_ATTEMPTS}/{settings.OAUTH_RATE_LIMIT_WINDOW}s", block=True
        )
    )
    def post(self, request):
        ...
        try:
            ...
            # Link Google account
            updated_user = oauth_service.link_google_account(request.user, token_result["user_info"])
            ...
            logger.warning(
                "Invalid OAuth state token for link operation", extra={"user_id": request.user.id, "ip": ip_address}
            )
            ...
            return error_response(
                "oauth_link_failed", [create_message("errors.oauth.link_failed", {})], status.HTTP_400_BAD_REQUEST
            )
...
class GoogleOAuthUnlinkView(APIView):
    ...
    def delete(self, request):
        ...
        if not request.user.check_password(password):
            logger.warning("Invalid password for OAuth unlink", extra={"user_id": request.user.id})
            return error_response(
                "invalid_password",
        ...
        try:
            ...
            return error_response(
                ...
                [create_message("errors.oauth.cannot_unlink_without_password", {"help_url": "/help/set-password"})],
                status.HTTP_400_BAD_REQUEST,
            )
