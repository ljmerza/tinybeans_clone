...
class OAuthInitiateRequestSerializer(serializers.Serializer):
    ...
    redirect_uri = serializers.URLField(required=True, help_text="The URI to redirect to after OAuth completes")


class OAuthInitiateResponseSerializer(serializers.Serializer):
    """Response serializer for OAuth initiation."""

    google_oauth_url = serializers.URLField(help_text="The Google OAuth URL to redirect the user to")
...
class JWTTokenSerializer(serializers.Serializer):
    ...
    refresh = serializers.CharField(help_text="JWT refresh token (long-lived)", required=False, allow_blank=True)
...
class OAuthCallbackResponseSerializer(serializers.Serializer):
    ...
    tokens = JWTTokenSerializer(help_text="JWT authentication tokens")
...
class OAuthUnlinkRequestSerializer(serializers.Serializer):
    ...
    password = serializers.CharField(
        required=True, write_only=True, style={"input_type": "password"}, help_text="User's password for verification"
    )
...
class OAuthErrorSerializer(serializers.Serializer):
    ...
    error = serializers.DictField(child=serializers.CharField(), help_text="Error details")
