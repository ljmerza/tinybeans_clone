...
def cleanup_expired_magic_login_tokens():
    ...
    deleted_count, _ = (
        MagicLoginToken.objects.filter(created_at__lt=cutoff_date)
        .filter(models.Q(is_used=True) | models.Q(expires_at__lt=timezone.now()))
        .delete()
    )
...
def cleanup_expired_oauth_states():
    ...
    remaining_expired = GoogleOAuthState.objects.filter(expires_at__lt=timezone.now()).count()

    if remaining_expired > 10000:
        logger.warning(f"OAuth state cleanup lag detected: {remaining_expired} expired states remaining")
