    ...
    def validate(redirect_uri: str, allowed_uris: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            for allowed_uri in allowed_uris:
                ...
                if (
                    parsed_uri.scheme == allowed_parsed.scheme
                    and parsed_uri.netloc == allowed_parsed.netloc
                    and parsed_uri.path.startswith(allowed_parsed.path)
                ):
                    logger.debug(
            ...
            logger.error(
                ...
                extra={"event": "oauth.redirect_uri_validation_error", "redirect_uri": redirect_uri, "error": str(e)},
                exc_info=True,
            )
            return False
    ...
    def validate_code_verifier(code_verifier: str) -> bool:
        ...
        if not all(c in allowed_chars for c in code_verifier):
            logger.warning(
                ...
                extra={"event": "oauth.pkce_validation_failed", "reason": "invalid_characters"},
            )
            return False
    ...
    def is_secure_token(token: str) -> bool:
        ...
        if len(token) < 128:
            logger.warning(
                ...
                extra={"event": "oauth.state_validation_failed", "reason": "token_too_short", "length": len(token)},
            )
            return False
        ...
        if unique_chars < 32:  # Should have good variety
            logger.warning(
                ...
                extra={"event": "oauth.state_validation_failed", "reason": "low_entropy", "unique_chars": unique_chars},
            )
            return False
