...
class TokenUtilityTests(TestCase):
    ...
    def test_token_data_integrity(self):
        ...
        payload = {"user_id": self.user.id, "email": self.user.email, "role": "test"}
...
class CookieUtilityTests(TestCase):
    ...
    @override_settings(
        SESSION_COOKIE_SECURE=True, SESSION_COOKIE_SAMESITE="Strict", SESSION_COOKIE_DOMAIN="example.com"
    )
