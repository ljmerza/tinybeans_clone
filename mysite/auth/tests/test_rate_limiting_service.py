...
class TestRateLimiting:
    ...
    def test_is_rate_limited_under_limit(self, settings):
        ...
        for i in range(2):
            create_code(user, code=f"12345{i}", expires_at=timezone.now() + timedelta(minutes=10))
    ...
    def test_is_rate_limited_expired_codes(self, settings):
        ...
        for i in range(5):
            code = create_code(user, code=f"12345{i}", expires_at=timezone.now() + timedelta(minutes=10))
