...
class AuthViewEdgeCaseTests(TestCase):
    ...
    def _response_fields(self, response):
        ...
        return [
            message.get("context", {}).get("field") for message in messages if message.get("context", {}).get("field")
        ]
    ...
    def test_password_reset_nonexistent_user(self):
        ...
        response = self.client.post(
            reverse("auth-password-reset-request"), {"email": "nonexistent@example.com"}, format="json"
        )
    ...
    def test_login_inactive_user(self):
        ...
        response = self.client.post(
            reverse("auth-login"), {"email": "inactive@example.com", "password": "password123"}, format="json"
        )
    ...
    def test_refresh_token_rotation(self):
        ...
        User.objects.create_user(email="token@example.com", password="password123")

        # Login to get initial tokens
        login_response = self.client.post(
            reverse("auth-login"), {"email": "token@example.com", "password": "password123"}, format="json"
        )
