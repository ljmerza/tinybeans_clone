...
class TestTrustedDeviceService:
    ...
    def test_add_trusted_device(self):
        ...
        # Check audit log
        log = TwoFactorAuditLog.objects.filter(user=user, action="trusted_device_added").first()
    ...
    def test_add_trusted_device_is_idempotent(self):
        ...
        first_device, first_token, first_created = TrustedDeviceService.add_trusted_device(user, request, days=30)
        second_device, second_token, second_created = TrustedDeviceService.add_trusted_device(user, request, days=30)
        ...
        refresh_log = TwoFactorAuditLog.objects.filter(user=user, action="trusted_device_refreshed").first()
    ...
    def test_add_trusted_device_max_limit(self):
        ...
        active_devices = TrustedDevice.objects.filter(user=user, expires_at__gt=timezone.now()).count()
    ...
    def test_remove_trusted_device(self):
        ...
        # Check audit log
        log = TwoFactorAuditLog.objects.filter(user=user, action="trusted_device_removed").first()
        assert log is not None
