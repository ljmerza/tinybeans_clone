...
from mysite.auth.models import TrustedDevice, TwoFactorSettings
...
class TestLoginWithTrustedDevice:
    ...
    def setup_method(self):
        ...
        self.twofa_settings = TwoFactorSettings.objects.create(user=self.user, is_enabled=True, preferred_method="totp")
    ...
    def test_login_skips_2fa_for_trusted_device(self, mock_get_device, mock_is_trusted):
        ...
        mock_get_device.return_value = TrustedDeviceToken(device_id="device-123", signed_value="signed-device-123")
...
class TestRememberMeFeature:
    ...
    def test_remember_me_creates_trusted_device(self, mock_verify, mock_add_device):
        ...
        response = self.client.post(
            "/api/auth/2fa/verify-login/", {"code": "123456", "partial_token": partial_token, "remember_me": True}
        )
