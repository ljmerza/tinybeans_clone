...
class TestTwoFactorSetupAPI:
    ...
    def test_setup_totp_success(self, mock_secret, mock_qr):
        ...
        response = self.client.post("/api/auth/2fa/setup/", {"method": "totp"})
    ...
    def test_setup_sms_missing_phone(self):
        ...
        response = self.client.post("/api/auth/2fa/setup/", {"method": "sms"})
    ...
    def test_setup_invalid_method(self):
        ...
        response = self.client.post("/api/auth/2fa/setup/", {"method": "invalid"})
...
class TestTwoFactorVerifySetupAPI:
    ...
    def test_verify_setup_totp_success(self, mock_email, mock_codes, mock_verify):
        ...
        # Create settings
        TwoFactorSettings.objects.create(user=self.user, preferred_method="totp", totp_secret="JBSWY3DPEHPK3PXP")

        # Mock verification
        mock_verify.return_value = True

        # Mock recovery codes
        mock_recovery_codes = [f"CODE-{i:04d}-TEST" for i in range(10)]
        mock_codes.return_value = mock_recovery_codes

        response = self.client.post("/api/auth/2fa/verify-setup/", {"code": "123456"})
    ...
    def test_verify_setup_email_success(self, mock_email, mock_codes, mock_verify):
        ...
        # Create settings
        TwoFactorSettings.objects.create(user=self.user, preferred_method="email")
    ...
    def test_verify_setup_invalid_code(self, mock_verify):
        ...
        response = self.client.post("/api/auth/2fa/verify-setup/", {"code": "999999"})
