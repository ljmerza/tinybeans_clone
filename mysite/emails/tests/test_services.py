...
from mysite.emails.template_loader import load_email_templates
...
class EmailServiceTests(SimpleTestCase):
    ...
    def test_send_email_unknown_template(self, mock_logger):
        ...
        mock_logger.error.assert_called_once_with("Unknown email template %s; dropping email", "unknown_template")
    ...
    def test_send_email_success_logging(self, mock_logger):
        ...
        mock_logger.info.assert_called_with("Dispatched %s email to %s", "test_template", "user@example.com")
...
{% block html_body %}<p>Hello {{ name }}</p>{% endblock %}"""
        ...
        with TemporaryDirectory() as tmp:
            ...
            with (
                patch.dict(EMAIL_TEMPLATE_FILES, {template_id: filename}, clear=False),
                patch("mysite.emails.template_loader._EMAIL_TEMPLATE_DIR", tmp_path),
            ):
                clear_registered_templates()
                load_email_templates()
