...
def success_response(
    data: dict[str, Any], messages: Optional[list[dict[str, Any]]] = None, status_code: int = status.HTTP_200_OK
) -> Response:
...
def error_response(
    error: str, messages: list[dict[str, Any]], status_code: int = status.HTTP_400_BAD_REQUEST
) -> Response:
    ...
    Example:
        ...
        ... )
    """
    return Response({"error": error, "messages": messages}, status=status_code)


def validation_error_response(
    messages: list[dict[str, Any]], status_code: int = status.HTTP_400_BAD_REQUEST
) -> Response:
...
def rate_limit_response(i18n_key: str = "errors.rate_limit", context: Optional[dict[str, Any]] = None) -> Response:
    ...
    """
    return error_response("rate_limit_exceeded", [create_message(i18n_key, context)], status.HTTP_429_TOO_MANY_REQUESTS)
