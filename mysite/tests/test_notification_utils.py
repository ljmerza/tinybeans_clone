...
class NotificationUtilsTestCase(TestCase):
    ...
    def test_create_message_with_context(self):
        ...
        msg = create_message("errors.file_too_large", {"filename": "photo.jpg", "maxSize": "10MB"})
    ...
    def test_success_response_with_messages(self):
        ...
        response = success_response(
            {"id": 123}, messages=[create_message("notifications.profile.updated")], status_code=status.HTTP_200_OK
        )
        ...
        self.assertEqual(response.data["messages"][0]["i18n_key"], "notifications.profile.updated")
    ...
    def test_validation_error_response(self):
        ...
        # Check first error
        self.assertEqual(response.data["messages"][0]["i18n_key"], "errors.email_invalid")
        self.assertEqual(response.data["messages"][0]["context"]["field"], "email")

        # Check second error
        self.assertEqual(response.data["messages"][1]["i18n_key"], "errors.password_too_short")
        self.assertEqual(response.data["messages"][1]["context"]["field"], "password")
        self.assertEqual(response.data["messages"][1]["context"]["minLength"], 8)
    ...
    def test_rate_limit_response_default(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_429_TOO_MANY_REQUESTS)
        self.assertEqual(response.data["error"], "rate_limit_exceeded")
        self.assertEqual(response.data["messages"][0]["i18n_key"], "errors.rate_limit")

    def test_error_response_no_data_property(self):
        """Test that error responses don't include a 'data' property"""
        response = error_response("test_error", [create_message("errors.test")], status.HTTP_400_BAD_REQUEST)
