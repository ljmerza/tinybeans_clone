...
- [x] **CORS / CSRF separation:** Move CORS allow-lists to environment variables (both dev and prod) and ensure `CORS_ALLOW_CREDENTIALS` only when the origin list is explicit. (Backed by `_env_list('CORS_ALLOWED_ORIGINS')` and conditional credential toggles.)
- [x] **Database & cache:** Configure `CONN_MAX_AGE`, SSL (`OPTIONS={'sslmode': 'require'}`) for Postgres, and add health checks for Redis. Consider `django-health-check` for operational readiness. (Implemented via `POSTGRES_CONN_MAX_AGE`, `POSTGRES_SSL_MODE`, and `django-health-check` apps.)
